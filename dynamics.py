...
def compute_aerodynamic_coefficients(state: State, params: Params) -> jnp.ndarray:
    ...
    return (
        ...
        + params.velocity_coefficients
        * state.kite.get_body_frame_velocity()
        * velocity_scale
    )
...
@jax.jit
def compute_tether_wrench(state: State, control: Control, params: Params) -> jnp.ndarray:
    ...
    for i, tether_state in enumerate(state.tethers):
        ...
        # in the world frame
        attachment_point = (
            state.kite.get_pose() @ params.tether_attachments[i]
        )
        ...
        tether_force = (
            tether_vector / jnp.linalg.norm(tether_vector) * control.tau[i]
        )
